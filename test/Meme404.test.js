...
describe("Meme", function () {
  ...
  it("should be update LFGToken", async function () {
    expect(await memeFactory.lfgToken()).to.equal(addr0); 
    await expect(memeFactory.setLFGToken(lfg.address))
    .to.emit(memeFactory, "LfgTokenUpdate")
    .withArgs(lfg.address); 
  ...
  it("should be getPrice", async function () {
    ...
    for (let i = 0; i < 100; i += 5) {
      let expectedPrice = toBn((i + 1) * (i + 1)).mul(oneEther).div(toBn(quadCurveA))
    ...
    for (let i = 0; i < 100; i += 5) {
      for (let amount in [1, 3, 5, 8, 12, 55, 99]) {
        ...
        for (let j = 0; j < amount; j++) {
          let price = toBn((i + 1 + j) * (i + 1 + j)).mul(oneEther).div(toBn(quadCurveA))
  ...
  it("should be newMemeClubWithQuadCurve(check arguments)", async function () {
    await expect(memeFactory.newMemeClubWithQuadCurve(1, "name", "symbol", "tokenUri", false, 0)).to.be.revertedWith("MemeClubLFGUnsupported()")
    await expect(memeFactory.newMemeClubWithQuadCurve(1, "name", "symbol", "tokenUri", true, 0)).to.be.revertedWith("MemeClubPriceArgs()")
  });
  ...
  it("should be buyCard (Native)", async function () { 
    ...
    for (let i = 0; i < 5; i++) {
      ...
      let expectedPrice = toBn(0)
      for (let j = 0; j < amount; j++) {
        let price = supply.add(1 + j) *  supply.add(1 + j).mul(oneEther).div(toBn(quadCurveA))
        expectedPrice = expectedPrice.add(price)
      }
      ...
      await expect(memeFactory.buyCard(clubId, amount, zeroEther, {value: expectedPrice}))
        .to.emit(memeFactory, "MemeClubTrade")
  ...
  it("should be pause", async function () {  
    ...
    await expect(memeFactory.newMemeClubWithQuadCurve(1, "name", "symbol", "tokenUri", true, quadCurveA)).to.emit(memeFactory, "MemeClubCreated")
    ...
    await expect(memeFactory.newMemeClubWithQuadCurve(1, "name", "symbol", "tokenUri", true, quadCurveA)).to.be.revertedWith("Pausable: paused")
